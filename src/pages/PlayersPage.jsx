...
export default function PlayersPage({ league }) {
  return (
    <>
      <PageTitle kicker="Plantillas" title="Jugadores" />
          ...
          return (
            <Link key={player.id} to={`/jugadores/${player.id}`} className="panel block overflow-hidden transition hover:-translate-y-1 hover:border-gold/50">
              ...
              <div className="p-4">
                ...
                {league.playerStatsById.get(player.id)?.activeSanctions.length > 0 && <div className="mt-3"><Badge tone="red">Sanción activa</Badge></div>}
              </div>
            </Link>
          )
        })}
